...
function parseExpectations(exp) {
    ...
    if(exp.rpId) {
        ...
        ret.set("rpId", exp.rpId)
    }

    // origin
    if (exp.origin) {
        if (typeof exp.origin !== "string" && !Array.isArray(exp.origin)) {
            throw new TypeError("expected 'origin' should be string or Array, got " + typeof exp.origin);
        }
